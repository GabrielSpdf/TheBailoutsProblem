...
cluster_folder = "./clusters_data/cluster"                     # Directory containing cluster files 
    ...
    with open(os.path.join(cluster_folder, file_name), 'r') as f:
        ...
        for line in f:
            ...
            for i, element in enumerate(elements):
                try:
                    if '.' in element:
                        # Convert to float if the element contains a decimal point
                        converted_elements.append(float(element))
                    else:
                        # Convert to int otherwise
                        converted_elements.append(int(element))
                except ValueError:
                    print(f"Erro ao converter o elemento '{element}' na linha: {line.strip()}")
                    continue
